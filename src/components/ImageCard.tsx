...
export const ImageCard = ({ 
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-white border border-pink-100 shadow-xl shadow-pink-200/40 p-1 group">
      ...
      <motion.div 
        className={cn("w-full h-full relative bg-slate-100 rounded-[22px] overflow-hidden", className)}
        initial={{ opacity: 0 }}
        animate={{ opacity: loaded || error ? 1 : 0 }}
        transition={{ duration: 0.5 }}
      >
        {error ? (
          <div className="absolute inset-0 bg-gradient-to-br from-color-primary-dark/20 to-color-neon-purple/20 flex flex-col items-center justify-center p-6 text-center">
            <ImageIcon className="w-8 h-8 text-white/30 mb-2" />
            <span className="text-xs font-semibold text-white/50 tracking-wider">SPARKLE MOMENT</span>
          </div>
        ) : (
          <img 
            src={src} 
